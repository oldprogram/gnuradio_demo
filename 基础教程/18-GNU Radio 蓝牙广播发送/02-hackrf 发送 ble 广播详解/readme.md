...
将 PSK 和 ASK 公式结合，并利用 $cos(α+β)=cosαcosβ-sinαsinβ$ 对其进行展开：
...
$$y_1 (t) = y_{I1}(t) + j y_{Q1}(t) = e^{j2 \pi F_{b1}t}$$ 

这样因为我们的信号是复数信号，我们能在频域区分出 $F_{b0}$ 和 $F_{b1}$，正是如此，我们也能设计所需要的滤波器和检测器。
